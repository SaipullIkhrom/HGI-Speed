...
import { FaCogs, FaEnvelope, FaLock, FaEye, FaEyeSlash, FaExclamationTriangle } from 'react-icons/fa';
...
const Login = () => {
  ...
  body: JSON.stringify(formData)
      ...
      if (res.ok && data.success) {
        ...
        setErrorMsg(data.message || t('login_failed'));
      }
    } catch {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center font-sans p-4 relative overflow-hidden selection:bg-[#e11d48] selection:text-white">
      ...
      <div className="bg-white border border-slate-100 p-8 md:p-10 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] w-full max-w-md space-y-8 relative z-10">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400 font-medium leading-relaxed px-4">{t('login_subtitle')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 text-xs">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#e11d48] hover:bg-red-700 disabled:bg-slate-300 disabled:text-slate-500 text-white font-black py-4 rounded-xl shadow-md hover:shadow-lg hover:shadow-red-500/30 transition-all active:scale-[0.98] tracking-widest uppercase mt-2 flex items-center justify-center"
          >
            {loading ? (
        ...
        <div className="pt-4 border-t border-slate-100 text-center">
          <p className="text-xs text-slate-500 font-medium">
            {t('no_account_text')} <span onClick={() => navigate('/register')} className="text-[#e11d48] font-black cursor-pointer hover:underline transition-all">{t('register_link')}</span>
          </p>
        </div>
